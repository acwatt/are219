...
- Either "attainment" or "non-attainment", \color{red}large penalties and costly restrictions
...
\[
\alert<2>{EPA_{i,t}} = \gamma_{i,0} + \sum\limits_{j\in J_i}\sum\limits_{k=1}^7\gamma_{j,k} 
\alert<3>{PA_{j,t}} \cdot \alert<4>{Winddir_{i,t,k}} + u_{i,t}
\]

\begin{itemize}
\item<1-> Analysis done at the month and quarter level; suppressing that subscript.
